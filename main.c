...
int main(void) {
    ...
    CD10 = (mic_time[1]-mic_time[0])/4000.0*343/2000.0;
    CD21 = (mic_time[2]-mic_time[1])/4000.0*343/4000.0;

    R_horz = ARRAY_LENGTH * ( 1 - fabs( (CD10 / ARRAY_LENGTH)*(CD10 / ARRAY_LENGTH)) ) + ARRAY_LENGTH * ( 1 - fabs( (CD21 / ARRAY_LENGTH ) * (CD21 / ARRAY_LENGTH) ));
    R_horz = R_horz / ( 2 * fabs( ( CD21 / ARRAY_LENGTH ) - ( CD10 / ARRAY_LENGTH ) ) );
    //R_horz = R_horz * 100;	//convert to cm

    B_horz = acos(( ARRAY_LENGTH*ARRAY_LENGTH - 2*R_horz*CD21 - CD21*CD21) / ( (2*R_horz*ARRAY_LENGTH) ))*180/3.14159;
